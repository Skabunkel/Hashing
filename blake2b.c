...
{
    int readBytes = BLAKE2B_CONSTANT_BLOCKBYTES;
    uint64_t bytesLeft = state->totalBlocks;
    ...
    while(bytesLeft > BLAKE2B_CONSTANT_BLOCKBYTES)
    {
        ...
        readBytes = state->totalBlocks - state->blockOfset > BLAKE2B_CONSTANT_BLOCKBYTES ? BLAKE2B_CONSTANT_BLOCKBYTES : messageLength-state->blockOfset;

        if(readBytes < BLAKE2B_CONSTANT_BLOCKBYTES)
        {
            memset(state->blocks, 0, BLAKE2B_CONSTANT_BLOCKBYTES);
        }
        bytesLeft = state->totalBlocks - state->blockOfset;
...
bool Blake2B(const uint8_t *message, const uint64_t messageLength, const uint8_t *key, const uint32_t keyLength, const uint8_t *salt, const uint32_t saltLength, const uint8_t *personalization, const uint32_t personalizationLength,  uint8_t *outbuffer, const uint32_t outLength)
{
    ...
    cs.blockOfset = 0;
    cs.totalBlocks = keyLength > 0 ? messageLength+128 : messageLength;
    ...
    if(keyLength > 0)
    {
        memcpy(messageBuffer, key, keyLength);
        ofset += 128;
    }

    memcpy(messageBuffer+ofset, message, messageLength);
