 ...
 *   based on this psudocode
 * https://en.wikipedia.org/wiki/BLAKE_%28hash_function%29#BLAKE2 and refrences
 * from https://github.com/BLAKE2/BLAKE2/
 *
 *   This implementation is not a full implimentation since it can only access
 * 2^64 bytes instead of the full 2^128 bytes the psudocode refrences For a safe
 * and tested implementation see https://github.com/BLAKE2/BLAKE2/
 *
 *   This implementation is for educational purposes and should be considered
 * unsafe for password hashing. I would like to make it safe, however i need to
 * read more about what that means and how it is achived.
 *
 *   I have not tested it on a big endian machine so i hope it will work. don't
 * know though. Now that i think about it probably not.
 */
...
void Mix(uint64_t *va, uint64_t *vb, uint64_t *vc, uint64_t *vd, uint64_t x,
         uint64_t y) {
  *va = *va + *vb + x;
  *vd = Shift64(*vd ^ *va, 32);
...
void Blake2B_Compress(Blake2BState *state, const bool isLastBlock) {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    Mix(&v[0], &v[4], &v[8], &v[12], messageVector[sigma[st][0]],
        messageVector[sigma[st][1]]);
    Mix(&v[1], &v[5], &v[9], &v[13], messageVector[sigma[st][2]],
        messageVector[sigma[st][3]]);
    Mix(&v[2], &v[6], &v[10], &v[14], messageVector[sigma[st][4]],
        messageVector[sigma[st][5]]);
    Mix(&v[3], &v[7], &v[11], &v[15], messageVector[sigma[st][6]],
        messageVector[sigma[st][7]]);

    Mix(&v[0], &v[5], &v[10], &v[15], messageVector[sigma[st][8]],
        messageVector[sigma[st][9]]);
    Mix(&v[1], &v[6], &v[11], &v[12], messageVector[sigma[st][10]],
        messageVector[sigma[st][11]]);
    Mix(&v[2], &v[7], &v[8], &v[13], messageVector[sigma[st][12]],
        messageVector[sigma[st][13]]);
    Mix(&v[3], &v[4], &v[9], &v[14], messageVector[sigma[st][14]],
        messageVector[sigma[st][15]]);
  }
...
bool Blake2B_Init(Blake2BState *state, const uint8_t outLength,
                  const uint8_t *key, const uint8_t keyLength,
                  const uint8_t *salt, const uint8_t saltLength,
                  const uint8_t *personalization,
                  const uint8_t personalizationLength) {
  ...
  uint8_t hashSt[BLAKE2B_CONSTANT_OUTANDKEYLENGTH] = {outLength, keyLength,
                                                      0x01, 0x01};
...
void Blake2B_Hash(Blake2BState *state, const uint8_t *message,
                  const uint64_t messageLength) {
  uint64_t bytesLeft = messageLength;
  uint64_t bytesRead = 0;

  int readBytes = 0;
  readBytes = messageLength > BLAKE2B_CONSTANT_BLOCKBYTES
                  ? BLAKE2B_CONSTANT_BLOCKBYTES
                  : messageLength;
  ...
  while (bytesLeft > BLAKE2B_CONSTANT_BLOCKBYTES) {
    Blake2B_Compress(state, false);
    readBytes = messageLength - bytesRead > BLAKE2B_CONSTANT_BLOCKBYTES
                    ? BLAKE2B_CONSTANT_BLOCKBYTES
...
void Blake2B_Finalize(Blake2BState *state, uint8_t *outBuffer,
                      const uint64_t outLength) {
...
bool Blake2B(const uint8_t *message, const uint64_t messageLength,
  ...
  if (outLength > BLAKE2B_CONSTANT_OUTANDKEYLENGTH)
    return false;
  if (keyLength > BLAKE2B_CONSTANT_OUTANDKEYLENGTH)
    return false;

  if (salt == NULL && saltLength > 0)
    return false;
  if (personalization == NULL && personalizationLength > 0)
    return false;

  if (saltLength > BLAKE2B_CONSTANT_SALT)
    return false;
  if (personalizationLength > BLAKE2B_CONSTANT_PERSONALIZATION)
    return false;
  ...
  if (!Blake2B_Init(&cs, (uint8_t)outLength, key, (uint8_t)keyLength, salt,
                    (uint8_t)saltLength, personalization,
                    (uint8_t)personalizationLength))
    return false;
